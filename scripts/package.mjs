...
async function npmMutate(spec, yes, update) {
    ...
    try {
        ...
        return await withRegistryLock(id, async () => {
            ...
            if (!ID_RE.test(id) || relative(extensionRoot, target) !== id) fail('invalid materialization path: extension id must be one direct child');
            ...
            try {
                ...
                if (!await confirm(yes, `${update ? 'Update' : 'Install'} ${id}?`)) fail('cancelled');
