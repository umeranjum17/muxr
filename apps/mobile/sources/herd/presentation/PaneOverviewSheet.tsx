...
export function PaneOverviewSheet(props: { visible: boolean; sessionId: string; onClose: () => void }): React.JSX.Element {
    ...
    const openPane = React.useCallback((pane: HerdrTreePane) => {
        ...
        rememberPaneSelection({ machineId, workspaceId: located.workspace.workspaceId, tabId: located.tab.tabId }, pane.sessionId);
        close();
                ...
                onPress: () => {
                    ...
                    sync.request('pane.close', { sessionId })
                        .then(async () => {
                            ...
                            const openable = panes.filter((entry) => entry.sessionId !== undefined);
                            const remaining = openable.filter((entry) => entry.sessionId !== sessionId);
                            const index = openable.findIndex((entry) => entry.sessionId === sessionId);
