...
export const TerminalScreen = React.memo((props: { id: string }) => {
     ...
     * without sending it.
     */
    const showLinkActions = React.useCallback((url: string) => {
        const safe = safeTerminalLinkUrl(url);
        setActionsOpen(false);
        setMenu({
            title: 'Link',
            note: displayLink(url, 96),
            items: [
                ...(safe === null ? [] : [{ label: 'Open', onPress: () => { void openExternalUrl(safe); } }]),
                { label: 'Copy', onPress: () => { void Clipboard.setStringAsync(url).then(() => showGestureHintRef.current('Link copied')); } },
                { label: 'Insert into the prompt', hint: INSERT_ONLY_LABEL, onPress: () => insertDraftRef.current(url) },
            ],
        });
    }, []);
    ...
    return (
        <ScopedTheme name="dark"><DarkSurface>{(theme) => {
            ...
            // The rail's two ends are one control size and one material, so
            // neither reads as the larger of the pair; only the trailing
            // control's own colour says which one acts. `railEnd` also keeps
            // both centred on the field's first line: the circles are shorter
            // than the field, and bottom-aligning them straight into the row
            // was what sat them a few points low of its centre.
            ...
            const artifactAction = railEnd(<Pressable onPress={attachPhotos} disabled={attaching} accessibilityRole="button" accessibilityLabel="Add attachment" accessibilityState={{ disabled: attaching }}
            ...
            // that floats over it gets counted as empty space and lands on the output.
                ...
                <View collapsable={false} style={{ flex: 1, backgroundColor: theme.colors.terminalChrome.canvas, paddingTop: insets.top, paddingBottom: keyboardVisible ? keyboardHeight : 0 }}>
                    ...
                    <View
                        ...
                        style={{ flex: 1 }}
                    >
                        <TerminalView sessionId={props.id} onStatus={onStatus} onChannel={onChannel} onViewControls={setViewControls} onLinkLongPress={showLinkActions} />
                        {gestureHint !== null && (
                            <View
                                pointerEvents="none"
                                style={{
