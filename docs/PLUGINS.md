...
`limits` renders one card that answers "can I start now, and when do I get more" from a bounded runtime payload: `{ "plan": "OpenCode Go", "verdict": "low", "windows": [ { "label": "Weekly", "window": "7d", "used": 90, "resetsIn": "4h 11m", "elapsed": 0.33, "pace": "low" } ] }`. The node's `title` and the payload's `plan` form the section's label row above the card, so the card opens with the verdict. The producer supplies `verdict` (`go`, `ahead`, `watch`, `low`, `limited`) and a percent-`used` share for each window; the app owns the wording, tones, and meters. Each window shows percent left, its reset, and optional `pace` (`limited`, `low`, `watch`, `ahead`, `on pace`, or `null` when the window has no usable length or reset, or has no usage yet). The built-in Usage host applies the same verdict rules to the headline and each calculable window; an unknown pace has no verdict text or warning colour of its own. When a window's own published figures (`used`, `elapsed`, `resetsIn`) project it running out before its reset, the app replaces the pace adjective with a short projection note (`out in 8h`), colours that figure accordingly, and steps the headline verdict up to match, so a calm verdict never stands over a warm figure. Meters fill with the share left (`100 - used`) against a 100% ceiling and show an optional static tick at the share of time left (`1 - elapsed`), so fill short of the tick means usage is ahead of pace. At most 8 windows with sanitized 24-byte labels, an 8-byte `window` suffix, `used` finite 0..100 (otherwise the window is dropped), `elapsed` finite 0..1 (otherwise omitted), and no colors in the payload. With no windows, the card collapses to the section label plus `message` as one quiet line, or the node's `emptyText` when there is no message; with neither, the card renders nothing. Requires `minMuxrVersion: 14`.
