...
Agent instructions
  muxr --skill | muxr skill       print the complete muxr agent skill and references
...
const COMMAND_HELP = {
    ...
    skill: `muxr --skill\nmuxr skill\n\nPrint the complete canonical muxr agent skill followed by every progressive-disclosure reference in deterministic order. The Herdr reference is generated from the installed binary at command time when available. No files or state are changed.\n`,
...
function printSkill() {
    ...
    const references = readdirSync(join(root, 'references'), { withFileTypes: true })
        ...
        .sort();
    const herdr = installedHerdrReference();
    ...
    for (const name of references) {
        ...
        const content = name === 'herdr.md' && herdr ? `${packaged}\n\n${herdr}` : packaged;
