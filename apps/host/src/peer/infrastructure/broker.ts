...
import type { DiagnosticOutcome, HostDiagnosticsJournal } from '../../diagnostics/index.js';
        ...
        socket.on('data', (chunk) => {
            ...
            void (async () => {
                try {
                    ...
                    socket.setTimeout(brokerRequestTimeoutMs(request), () => socket.destroy());
                    ...
                    try {
                        ...
                        const outcome = brokerDiagnosticOutcome(message);
...
function brokerRequestTimeoutMs(request: PeerBrokerRequest): number {
    if (request.method === 'watch') {
        return Math.min(Math.max(Math.trunc(request.timeoutMs ?? 30_000), 1_000), 290_000) + 25_000;
    }
    if (request.method === 'prompt') return PEER_MUTATION_TTL_MS + 25_000;
    return 45_000;
}

function brokerDiagnosticOutcome(message: string): DiagnosticOutcome {
