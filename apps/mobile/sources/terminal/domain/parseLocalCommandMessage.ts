 ...
 *
 * muxr shows the user's sent message optimistically (it carries a
 ...
 * Some terminal agents render slash commands.
 *
 * Gated on `hasLocalId` so we only ever hide a message the user actually
 * sent from muxr, never an agent/SDK-originated one.
 */
