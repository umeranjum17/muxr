...
export function resolveStatusBarGitBranch(
    ...
    // Neither source is populated for Pi sessions today, so a worktree would
    // show no branch at all — and the branch chip is what lands it. The
    // worktree directory is named after its branch, so the path carries it.
    return worktreeBranchFromPath(sessionPath);
}

function worktreeBranchFromPath(path: string | null | undefined): string | null {
    if (!path) return null;
...
export function getUsageLimitRows(limits: UsageLimitsLike): UsageLimitRow[] {
    ...
    const sorted = [...limits.windows].sort((a, b) => {
        ...
        const aOrder = ai < 0 ? known.length : ai;
        const bOrder = bi < 0 ? known.length : bi;
