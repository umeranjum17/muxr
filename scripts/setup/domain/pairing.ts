...
/** Native and explicitly durable browser-control grants last until revocation. */
export const DURABLE_GRANT_EXPIRES_AT = Date.UTC(9999, 11, 31, 23, 59, 59, 999);
export const BROWSER_GRANT_TTL_MS = 8 * 60 * 60_000;
/**
 * Legacy personal-browser records retain their reviewed 30-day lifetime.
 * New control invitations carry the separate durable consent marker.
 */
...
export type PairingIntent = {
    ...
    personal: boolean;
    /** New browser-control approval reviewed as lasting until revocation. */
    durable: boolean;
    requiresWebHosting: boolean;
    grantExpiresAt: (now?: number) => number;
    refreshExpiresAt: (existingIso: string, now?: number) => number;
    /** Human duration for consent copy. */
    grantDurationLabel: () => string;
    matchesPending: (pending: { deviceKind?: unknown; authority?: unknown; personal?: unknown; durable?: unknown }) => boolean;
    deviceRecord: (fields: {
        deviceId: string;
...
export function pairingIntent(input: { kind?: unknown; authority?: unknown; personal?: unknown; durable?: unknown }): PairingIntent {
    ...
    // An omitted marker means a newly printed control invitation. Stored
    // records and pending sessions pass an explicit false when old/absent.
    const durable = kind === 'browser' && authority === 'control' && !personal && input.durable !== false;
    ...
    return Object.freeze({
        ...
        grantExpiresAt(now = Date.now()) {
            return kind === 'browser' && !durable ? now + ttlMs : DURABLE_GRANT_EXPIRES_AT;
        },
        refreshExpiresAt(existingIso: string, now = Date.now()) {
            if (kind !== 'browser') return DURABLE_GRANT_EXPIRES_AT;
            if (durable) return Math.min(Date.parse(existingIso), DURABLE_GRANT_EXPIRES_AT);
            return Math.min(Date.parse(existingIso), now + ttlMs);
        },
        grantDurationLabel() {
            if (kind !== 'browser') return 'until revoked';
            if (durable) return 'until revoked';
            return personal ? '30 days' : 'eight hours';
        },
        matchesPending: (pending: { deviceKind?: unknown; authority?: unknown; personal?: unknown; durable?: unknown }) => {
            ...
            return pendingKind === kind && pendingAuthority === authority
                && (pending.personal === true) === personal && (pending.durable === true) === durable;
        },
        ...
        pairingLocator(relayUrl, code, machineName) {
            ...
            if (kind === 'browser') {
                ...
                if (durable) locator.searchParams.set('durable', '1');
...
export function pairingIntentFromHostedFlags(args: readonly string[]): PairingIntent {
    ...
    // The hosted authority still issues eight-hour credentials. Only the
    // self-host relay persists the durable browser consent marker.
    return pairingIntent({ kind: browser ? 'browser' : 'native', authority: observe ? 'observe' : 'control', durable: false });
}

export function pairingIntentFromPairFlags(args: readonly string[]): PairingIntent {
    const browser = args.includes('--browser') || args.includes('--browser-view') || args.includes('--browser-personal');
    return pairingIntent({
        kind: browser ? 'browser' : 'native',
        authority: args.includes('--browser-view') ? 'observe' : 'control',
        personal: args.includes('--browser-personal'),
    });
}
...
export function pairingIntentFromDevice(device: { kind?: unknown; authority?: unknown; personal?: unknown; durable?: unknown }): PairingIntent {
    return pairingIntent({
        kind: device.kind === 'browser' ? 'browser' : 'native',
        authority: device.authority,
        personal: device.personal,
        durable: device.durable === true,
    });
}
