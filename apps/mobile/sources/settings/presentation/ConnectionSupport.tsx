...
export function ConnectionSupport({ hostVersion: reportedHost }: { hostVersion?: string }) {
    ...
    return <>
        <ItemGroup title="Installed versions">
            {/* A version difference is a fact on the row that acts on it: one
                warning mark, no second bordered surface competing for attention. */}
        ...
        <ItemGroup title="Troubleshooting" footer="Diagnostics contain durations, counts and status codes. Credentials, terminal content and private identifiers are excluded.">
            ...
            {details !== undefined && <>
                <Text selectable style={{ marginHorizontal: 16, marginBottom: 12, color: theme.colors.textSecondary, fontSize: 12, lineHeight: 18, ...Typography.mono() }}>{details}</Text>
