---
name: muxr
description: Set up and operate muxr (control coding agents from your phone) — open pages on the computer's desktop browser for the user to watch through Computer, use durable Shared Artifacts, install/pair/self-host, drive Herdr workspaces/panes/agents/worktrees, hand browser login/2FA/CAPTCHA to the phone, connect computers for cross-machine collaboration and voice, and author/install muxr plugins. Use for any muxr or Herdr setup, orchestration, collaboration, plugin, or troubleshooting task.
license: Apache-2.0
...
- Open web pages in the computer's normal desktop browser; the user watches
  and takes over through the phone's Computer action. Load
