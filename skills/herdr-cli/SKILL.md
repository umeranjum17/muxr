---
name: "herdr-cli"
description: "Drive herdr (terminal workspace manager) from the CLI or its socket API — create workspaces/tabs/panes, start and orchestrate coding agents, read pane output and scrollback, wait on lifecycle or output. Use when scripting herdr, spawning helper agents, reading another agent's transcript, or building against the herdr socket API."
version: 1
created: "2026-08-05"
updated: "2026-08-05"
---
## When to Use
Use when the task involves controlling herdr: creating workspaces/tabs/panes, launching or prompting coding agents in panes, reading terminal output or agent transcripts, waiting on an agent to finish or for text to appear, or integrating an app against the herdr socket API (~/.config/herdr/herdr.sock). Also use when a herdr call returns an unexpected empty result or an `agent_not_idle` / `single_pane` style outcome. Not for ordinary local shell work that has no herdr involvement.

**Routing rule:** when the user asks you to spawn agents, tabs, panes, workspaces, or worktrees, that is ALWAYS herdr (you are running inside one of its panes). Do not fall back to pi's subagent tool for this — subagents don't get panes, can't be watched from the phone, and die with your session.

## Spawning things from inside a pane (the common ask)
Every herdr pane exports `HERDR_PANE_ID` (e.g. `w2A:p4`). That one value is the whole map: **workspace id = `${HERDR_PANE_ID%%:*}`** (the part before the colon), and `herdr pane get $HERDR_PANE_ID` returns `.result.pane.tab_id` + `.result.pane.workspace_id`. Recipes, all verified against herdr 0.8.0:
...
`agent start` needs the pane at a shell prompt and BLOCKS until herdr detects the agent (up to ~30s+); pass a generous `--timeout`.
