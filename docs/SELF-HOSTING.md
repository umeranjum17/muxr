...
SSH is only a route to the loopback relay. It does not authorize a device, replace a grant, or remove E2EE. The desktop picture and controls use WebRTC, not that relay: directly when the phone can reach the computer, and otherwise over TCP through a second forward on the same SSH connection, so a phone that can reach only SSH (port 22) still views and controls the desktop. Tailscale Serve remains the recommended default because it needs less per-device credential setup and reconnects without a separate SSH session.
