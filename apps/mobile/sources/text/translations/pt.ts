...
export const pt: TranslationStructure = {
    ...
    plugins: {
        ...
        stopDictationHint: 'Para a gravação e começa a transcrever',
        ...
        herdrAndMuxrFooter: 'O Herdr executa ações, painéis ou eventos para estes, que acrescentam interface muxr.',
        muxrOnly: 'Apenas muxr',
        muxrOnlyFooter: 'O Herdr apenas os regista; tudo o que fazem passa pelo muxr.',
        herdrOnly: 'Apenas Herdr',
        herdrOnlyFooter: 'Pacotes de backend sem interface muxr. Faça a gestão com a CLI herdr.',
        ...
        limits: {
            ...
            low: 'Quase esgotado',
            limited: 'Limitado',
            paceLimited: 'Limitado',
            paceExhausted: 'Esgotado',
            paceOnTrack: 'No ritmo',
            paceAhead: 'Adiantado',
            paceBurning: 'Consumo rápido',
        ...
        rightNow: {
            ...
            refreshFailed: 'Não foi possível atualizar · toque para tentar de novo',
            refreshNow: 'Atualizar uso agora',
            refreshThrottled: ({ seconds }: { seconds: number }) => `Atualização disponível em ${seconds} s`,
            refreshIn: ({ seconds }: { seconds: number }) => `em ${seconds} s`,
    ...
    recentActivity: {
        ...
        readyUnseen: 'Concluído · não visto',
        showMore: ({ count }: { count: number }) => `Mostrar ${count} mais`,
    ...
    spacesTree: {
        ...
        untitledWorkspace: 'Espaço de trabalho sem nome',
        groupCount: ({ count, kind }: { count: number; kind?: string }) => {
            if (kind === 'task') return `${count} ${plural({ count, singular: 'tarefa', plural: 'tarefas' })}`;
    ...
    commandPalette: {
        ...
        agentCommands: ({ agent }: { agent: string }) => `Comandos de ${agent}`,
        common: 'Comuns',
        allCommands: ({ kind }: { kind: string }) => `Todos os comandos de ${kind}`,
        destructive: 'Destrutivos · pedem confirmação',
        custom: 'Personalizado',
        typeCommand: 'Digite um comando…',
        insertSlash: 'Insere / no compositor',
        noMatch: 'Nenhum comando corresponde',
        noCatalogue: ({ kind }: { kind: string }) => `Ainda não há catálogo para ${kind} · digite um comando abaixo`,
    ...
    sessionArtifacts: {
        title: ({ count }: { count: number }) => count === 1 ? '1 artefato' : `${count} artefatos`,
    },
    ...
    files: {
        ...
        toggleFileAndDiff: 'Alternar arquivo e diff',
        ...
        previousFileNamed: ({ title, ordinal, total }: { title: string; ordinal: number; total: number }) => `Arquivo anterior, ${title}, ${ordinal} de ${total}`,
        nextFileNamed: ({ title, ordinal, total }: { title: string; ordinal: number; total: number }) => `Próximo arquivo, ${title}, ${ordinal} de ${total}`,
        filePosition: ({ current, total }: { current: number; total: number }) => `Arquivo ${current} de ${total}`,
    ...
    browser: {
        ...
        statusWaitingForYou: ({ agent }: { agent: string }) => `${agent} está aguardando seu login`,
        statusStopped: ({ agent }: { agent: string }) => `${agent} parou`,
        statusIdleBrowser: ({ agent }: { agent: string }) => `Navegador de ${agent}`,
        waitingForConnection: ({ machine }: { machine: string }) => `Aguardando a conexão com ${machine}…`,
        lostBody: ({ machine }: { machine: string }) => `A página pode continuar aberta em ${machine}.`,
        openFailedTitle: ({ machine }: { machine: string }) => `Não foi possível abrir um navegador em ${machine}.`,
        noDriverTitle: ({ machine }: { machine: string }) => `Falta o driver do navegador em ${machine}.`,
        noDriverBody: 'Instale-o uma vez naquele computador e reabra esta tela.',
        installBody: ({ machine }: { machine: string }) => `Nenhum navegador está instalado em ${machine}. Execute isto uma vez naquele computador (cerca de 500 MB) e tente novamente.`,
        copyCommand: 'Copiar comando',
        copied: 'Copiado',
        copyFailedTitle: 'Não foi possível copiar',
        copyFailedBody: 'Tente novamente.',
        goTo: 'Ir para',
        goToPlaceholder: 'Site ou endereço',
        goToConfirm: 'Ir',
        goToFailed: 'Não foi possível abrir esse endereço',
    },
