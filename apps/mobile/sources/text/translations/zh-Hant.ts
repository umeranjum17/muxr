...
export const zhHant: TranslationStructure = {
    ...
    plugins: {
        ...
        herdrAndMuxrFooter: 'Herdr 為它們執行操作、窗格或事件掛鉤，它們也提供 muxr 介面。',
        muxrOnly: '僅 muxr',
    ...
    files: {
        ...
        nextFileNamed: ({ title, ordinal, total }: { title: string; ordinal: number; total: number }) => `下一個檔案，${title}，第 ${ordinal} 個，共 ${total} 個`,
        filePosition: ({ current, total }: { current: number; total: number }) => `檔案 ${current} / ${total}`,
        diffUnavailable: '差異不可用，此檔案沒有變更',
