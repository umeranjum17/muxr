...
The completion summary reports relay location, relay URL, local host and Herdr health, credential expiry, pairing outcome, integrations/plugins, and `~/.muxr`. It never reports internal ids, credentials, or keys.
...
10. Update docs and package smoke.
...
- `apps/relay/src/selfhostPairing.ts`: slug-scoped pair/device/grant operations.
...
- [x] Native QR pairing and clickable read-only browser pairing retain server-bound authority and secure-context checks.
- [x] Final summaries expose URLs and health, never ids, credentials, claims, or keys.
- [x] Existing local self-host flow, 28-gate suite, package smoke, and three independent adversarial reviews are green.
