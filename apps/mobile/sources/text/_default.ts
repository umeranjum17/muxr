...
export const en = {
    ...
    plugins: {
        ...
        limits: {
            ...
            paceLimited: 'Rate limited',
            paceExhausted: 'Exhausted',
            paceOnTrack: 'On pace',
        ...
        rightNow: {
            ...
            refreshIn: ({ seconds }: { seconds: number }) => `in ${seconds}s`,
    ...
    files: {
        ...
        nextFileNamed: ({ title, ordinal, total }: { title: string; ordinal: number; total: number }) => `Next file, ${title}, ${ordinal} of ${total}`,
        filePosition: ({ current, total }: { current: number; total: number }) => `File ${current} of ${total}`,
        ...
        goToPath: ({ label }: { label: string }) => `Go to ${label}`,
        fullPath: 'Full path',
