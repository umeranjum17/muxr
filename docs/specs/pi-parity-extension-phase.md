...
- `extension.call` input is capped at 8 KiB of JSON, complete RPC stdout is limited to 64 KiB (rejected when exceeded, never silently truncated before parsing/transport), and rendered RPC text is capped at 4 KiB before reaching React Native.
...
- Lower whole-file `attachment.fetch` healing to 2 MiB so base64 JSON stays below the hosted 4 MiB frame cap; larger files use existing chunked/download paths. Limit one sheet-open heal burst to 8 files and 4 MiB total.
