...
describe('Herdr graphics flow', () => {
    ...
    // One flow check for the adaptive density the direct path uses: a large
    // frame is halved while the pane is busy and refined once it settles.
    it('halves a large direct frame while moving, then refines it after the pane settles', async () => {
        ...
        // 600x400 is past the size where halving the density is worth it.
        const width = 600;
        const height = 400;
        ...
        const dir = mkdtempSync(join(tmpdir(), 'herdr-adaptive-direct-'));
        const path = join(dir, 'frame.rgba');
        writeFileSync(path, rgba);
        const drain = vi.spyOn(internals, 'drain');
        try {
            internals.enqueue({
                path, expectedLength: BigInt(rgba.length), imageId: 10, transferId: 10n,
                leading: Buffer.from('\u001b[1;1H'), control: `a=T,f=32,s=${width},v=${height},i=10`,
            });
            await drain.mock.results.at(-1)?.value;
            const halved = frameAnsi(frames[0]!);
            expect(halved).toContain('a=T,f=32,s=300,v=200,');
            // Cell counts are independent of pixel density: the placement is
            // the one the phone would have had at full density.
            expect(halved).toContain('c=20,r=7');
            const encoded = [...halved.matchAll(/\u001b_G[^;]*;([^\u001b]*)\u001b\\/g)]
