...
function validIntent(value: unknown): value is CollaborationIntent {
    if (!value || typeof value !== 'object') return false;
    const intent = value as Partial<CollaborationIntent>;
    return intent.version === 1 && Array.isArray(intent.selectedMachineIds) && Array.isArray(intent.machines) && Array.isArray(intent.edges)
        && intent.selectedMachineIds.every((id) => typeof id === 'string' && id !== '')
        && intent.machines.every((machine) => typeof machine?.machineId === 'string' && typeof machine?.name === 'string'
            && typeof machine?.machineSigningPublicKey === 'string')
        && intent.edges.every((edge) => typeof edge?.sourceMachineId === 'string' && typeof edge?.targetMachineId === 'string'
...
export async function loadCollaborationIntent(): Promise<CollaborationIntent> {
    ...
    try {
        ...
        return validIntent(parsed) ? parsed : emptyIntent();
    } catch {
        return emptyIntent();
    }
}
...
export function selectCollaborationMachines(
    ...
    const next = cloneIntent(current);
