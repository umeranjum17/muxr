...
|---|---|---|---|
| Admit a client frame | [`admitClientFrame`](./contract/src/control-plane/application/admitClientFrame.ts) | Client Frame | `apps/host/src/machine/infrastructure/linkEndpoint.ts` |
...
| Watch Agent Lifecycle | `agent.watch` | Agent Watch, Lifecycle Event | dispatcher, `apps/mobile/sources/watch/agentWatch.ts` |
| Focus Agent | `pane.focus` | Agent Route | dispatcher, `herdrSessionSource.ts` |
| Reconnect Machine | stored link grant | Device Grant | `linkFirstClient.ts`, `sync.ts` |
...
| Open Terminal | byokit link stream | Terminal Frame | `OpenTerminal.ts`, `linkEndpoint.ts` |
...
Throwing aliases (`parseClientFrame`, `parseManifest`, `normalizePairingCode`, `verifyDeviceGrant`) stay for existing adapters. New code should call the named use case.
