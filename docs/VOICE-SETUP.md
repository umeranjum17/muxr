...
Exactly one adapter runs at a time. The selection is the plugin's own state under `MUXR_PLUGIN_STATE_DIR`, read by its `voice.provider.list` and `voice.provider.set` capabilities, so switching providers is not an enable/disable of separate packages. In the app, open **Settings → Voice & dictation** to switch. Grok, Gemini Live, and OpenAI Realtime collect their API key through Configure. Codex Voice uses the existing local `codex login`.
