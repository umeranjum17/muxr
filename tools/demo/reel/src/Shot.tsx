...
import { DISPLAY, MONO, muted, text } from './theme';
...
export type ShotSpec = {
    ...
    flipTo?: 'light' | 'dark';
    camera?: CameraKey;
    /** `desk` puts the same pane in its desktop window behind the handset. */
    layout?: 'split' | 'full' | 'desk';
    side?: 'left' | 'right';
};
...
export const Shot: React.FC<{ spec: ShotSpec; durationInFrames: number }> = ({ spec, durationInFrames }) => {
    ...
    const full = layout === 'full' || (portrait && !desk);
            ...
            <div style={{ overflow: 'hidden', paddingBottom: 14 }}>
                <div
                    style={{
                        ...
                        fontWeight: 700,
                        color: text,
                        fontSize: portrait ? 88 : (full ? 96 : 106),
                        lineHeight: 1.02,
        ...
        return (
            <Backdrop haloAt="64%">
                <AbsoluteFill style={{ opacity: reveal(frame, 0, 16) }}>
                    ...
                    >
                        <Desk theme={spec.theme} t={t} delay={8} />
                    </div>
                    <div
                        style={{
                            position: 'absolute',
