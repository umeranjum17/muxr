...
export const Item = React.memo<ItemProps>((props) => {
    ...
    // If copy is enabled and no onPress is provided, a plain tap copies —
    // the long-press shortcut alone is undiscoverable. A row with its own
    // onLongPress keeps tap a no-op so it doesn't fight the press handler.
    const handlePress = onPress !== undefined
        ? onPress
        : (copy && !isWeb && onLongPress === undefined ? () => { void handleCopy(); } : undefined);

    const isInteractive = handlePress || onLongPress || (copy && !isWeb);
    // A chevron promises a tap does something: only show it with a real
    // press handler, not for copy-only rows.
    const showAccessory = (onPress !== undefined || onLongPress !== undefined) && showChevron && !rightElement;
    ...
    if (isInteractive) {
        return (
            <BubblePressable
                ...
                accessibilityState={{ disabled: !!(disabled || loading), selected: !!selected }}
