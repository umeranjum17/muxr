...
try {
    ...
    const absentServer = createServer();
    await new Promise((resolve) => absentServer.listen(0, '127.0.0.1', resolve));
    const absentPort = absentServer.address().port;
    await new Promise((resolve) => absentServer.close(resolve));
    await refused('session.navigate', { session, url: `http://127.0.0.1:${absentPort}/` });
    const loadFailure = await ok('session.status', { session, deviceId });
    assert(loadFailure.reason === 'The dev server is not running on this computer.', `load failure: ${JSON.stringify(loadFailure)}`);
    const navigated = await ok('session.navigate', { session, url: `${origin}/login` });
    assert(navigated.site === '127.0.0.1', 'site after navigation');
    const recovered = await ok('session.status', { session, deviceId });
    assert(recovered.reason === undefined, 'successful navigation clears the load failure');
    ...
    await signal(second, 1, { type: 'hello' });
    const otherKey = generateKeyPair();
    await ok('device.enroll', { deviceId: 'dev_other', devicePublicKey: otherKey.publicKey });
    await refused('session.signal', { session: second, deviceId: 'dev_other', generation: 1, message: 'e2ee:v2:xx' });
    assert((await signal(second, 1, { type: 'heartbeat' })).status.state === 'agent-driving', 'invalid second-device signal leaves the current peer intact');
    ...
    assert(status.state === 'agent-driving', 'asking for help keeps the shared browser live');
    assert((await ok('session.snapshot', { session: second })).text.includes('Continue'), 'agent can still inspect the shared tab after asking');
