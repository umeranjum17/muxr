...
function wellFormedBrowserPairQuery(parsed: URL): boolean {
    const codes = parsed.searchParams.getAll('pair');
    const roles = parsed.searchParams.getAll('role');
    const role = roles[0];
    const personal = parsed.searchParams.getAll('personal');
    const durable = parsed.searchParams.getAll('durable');
    const knownKeys = [...parsed.searchParams.keys()].every((key) => key === 'pair' || key === 'role' || key === 'personal' || key === 'durable' || key === 'name');
    return codes.length === 1
        && codes[0] !== ''
        && roles.length === 1
        && (role === 'control' || role === 'observe')
        && (personal.length === 0 || (personal.length === 1 && personal[0] === '1'))
        && (durable.length === 0 || (durable.length === 1 && durable[0] === '1' && role === 'control' && personal.length === 0))
        && knownKeys
        && wellFormedConsentName(parsed.searchParams)
        && parsed.hash === '';
}
...
/**
 * The lifetime the link asks for. New control approvals explicitly say
 * until revoked; old and view-only browser links keep their finite consent.
 */
export function hostedPairingLifetime(url: string): 'eight hours' | '30 days' | 'until revoked' {
    const fragment = pairingSearchParams(url);
    if (fragment.get('durable') === '1') return 'until revoked';
    if (fragment.get('personal') === '1') return '30 days';
    const decoded = compactPairingRecord(fragment.get('payload'));
    if (decoded?.durable === '1') return 'until revoked';
...
const BROWSER_QR_KEYS = ['pair', 'role', 'personal', 'durable', 'name'];
...
export function parseBrowserPairingQr(value: unknown): BrowserPairingQrParse {
    ...
    const durable = parsed.searchParams.get('durable');
    ...
    if (durable !== null && (durable !== '1' || role !== 'control')) return { ok: false, error: NOT_A_BROWSER_QR };
    if (personal !== null && durable !== null) return { ok: false, error: NOT_A_BROWSER_QR };
    if (!wellFormedConsentName(parsed.searchParams)) return { ok: false, error: NOT_A_BROWSER_QR };
    return { ok: true, qr: { url: value, origin: parsed.origin, authority: role, personal: personal === '1', durable: durable === '1', displayName: pairingDisplayNameOf(value) } };
}

export type ReviewedConsent = { name: string; lifetime: 'eight hours' | '30 days' | 'until revoked' };
...
export function reviewedConsentMismatch(reviewed: ReviewedConsent, sealed: URLSearchParams): string | undefined {
    ...
    if (sealed.get('durable') !== null && (sealed.get('durable') !== '1' || sealed.get('authority') !== 'control')) {
    ...
    const sealedLifetime = sealed.get('durable') === '1' ? 'until revoked' : sealed.get('personal') === '1' ? '30 days' : 'eight hours';
