...
In the native app: **Scan QR code** or enter the short relay-qualified pairing string. Browser pairing prints one short two-minute HTTPS link. `muxr pair --browser` grants full terminal and agent control until that device is revoked; `muxr pair --browser-view` grants explicit view-only access for eight hours. Both survive refresh/restart and are reported as paired only after durable browser storage acknowledges the grant. Existing browser grants retain their original expiry.
