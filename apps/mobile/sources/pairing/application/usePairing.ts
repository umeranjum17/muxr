...
import { hostedPairingAuthority, hostedPairingDisplayName, hostedPairingLifetime, prepareHostedPairingInput } from './hostedE2ee';
    ...
    return React.useCallback(async (url: string) => {
        ...
        try {
            ...
            const approved = await Modal.confirm(
                ...
                (Platform.OS === 'web'
                    ? `This browser receives ${browserAuthority === 'control' ? 'full terminal and agent control' : 'view-only access'} for ${hostedPairingLifetime(prepared)}. Machine keys stay end-to-end encrypted with WebCrypto in this browser.\n\nOnly continue if you just ran \`${browserAuthority === 'control' ? 'muxr pair --browser' : 'muxr pair --browser-view'}\` there.`
