...
describe('Herdr graphics flow', () => {
    it('decodes, routes, and emits one pane-scoped Kitty frame', () => {
        ...
        // Herdr 0.8.2 downgrades DECSET 1016 to 1-based cell SGR. Phone taps
        // must emit the same encoding, including when the client viewport
        // is not an exact cell-grid multiple.
        expect(mapGraphicsPointer(image, portrait, { phase: 'down', x: 200, y: 50, width: 400, height: 1600 })).toBeUndefined();
        expect(mapGraphicsPointer(image, portrait, { phase: 'down', x: 200, y: 790, width: 400, height: 1600 })).toEqual({ x: 21, y: 40 });
        expect(mapGraphicsPointer(image, portrait, { phase: 'down', x: 205, y: 810, width: 410, height: 1640 })).toEqual({ x: 21, y: 40 });
        ...
        expect(mapGraphicsPointer(image, landscape, { phase: 'down', x: 595, y: 300, width: 1200, height: 600 })).toEqual({ x: 60, y: 16 });
