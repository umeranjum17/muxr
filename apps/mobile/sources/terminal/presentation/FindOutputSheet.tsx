...
export function FindOutputSheet({ sessionId, onClose }: { sessionId: string; onClose: () => void }) {
    ...
    const context = current === undefined ? [] : lines.slice(Math.max(0, current - 3), Math.min(lines.length, current + 4));
    const contextStart = current === undefined ? 0 : Math.max(0, current - 3);
    ...
    return <View accessibilityViewIsModal style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, zIndex: 50, justifyContent: 'flex-end', backgroundColor: theme.colors.scrim }}>
        ...
        <View style={{ backgroundColor: theme.colors.surface, borderTopLeftRadius: 16, borderTopRightRadius: 16, paddingBottom: insets.bottom + 12, maxHeight: '85%' }}>
            ...
            <TextInput value={query} onChangeText={(value) => { setQuery(value); setSelected(0); }} autoFocus autoCorrect={false}
                ...
                style={{ marginHorizontal: 16, borderRadius: 8, padding: 12, color: foreground, backgroundColor: theme.colors.surfaceHigh }} />
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, paddingHorizontal: 16, paddingVertical: 12 }}>
                <Text style={{ flex: 1, color: secondary, fontSize: 13 }}>
            ...
            {current !== undefined && <ScrollView style={{ maxHeight: 240, marginHorizontal: 16, marginBottom: 8, borderRadius: 8, backgroundColor: theme.colors.surfaceHigh }}>
                {context.map((line, offset) => <Text key={contextStart + offset} selectable style={{ color: contextStart + offset === current ? foreground : secondary, backgroundColor: contextStart + offset === current ? theme.colors.surfaceSelected : 'transparent', fontFamily: 'monospace', fontSize: 12, paddingHorizontal: 10, paddingVertical: 4 }}>
                    {`${contextStart + offset + 1}  ${line}`}
                </Text>)}
            </ScrollView>}
            {truncated && <Text style={{ color: secondary, fontSize: 11, marginHorizontal: 16, marginBottom: 4 }}>Showing the most recent available output</Text>}
        </View>
    </View>;
}
