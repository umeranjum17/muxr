        ...
        <View style={[styles.wrapper, style]}>
            <View style={styles.container}>
                ...
                <View style={[styles.contentContainer, containerStyle]}>
                    {(() => {
                        // Count only real, visible rows: conditional rendering
                        // leaves false/null children behind, and counting them
                        // leaves a dangling divider on the last visible row.
                        const items = React.Children.toArray(children).filter(
                            (child): child is React.ReactElement<ItemChildProps> => React.isValidElement<ItemChildProps>(child)
                        );
                        const totalRows = items.filter((child) => child.type !== React.Fragment).length;
                        let rowIndex = -1;
