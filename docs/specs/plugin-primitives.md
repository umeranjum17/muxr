...
- Shell slots mount primitives without naming individual plugin products; `_layout` mounts only kernel notification infrastructure
...
- Item-list UI verification proves distinct Changes/Files pill icons, git-status and MIME-aware row icons, green additions/red deletions, file and attachment row dispatch, and an independent temporary plugin’s sheet-level action dispatch. A 177,321,373-byte APK also downloads fully over E2EE chunks, matches its source SHA-256, and reaches Android’s share handoff without loading the blob into JS memory.
...
- UI version 10 device acceptance proves File Viewer starts as a compact folder tree, lazily expands deep folders, collapses/expands loaded branches, and opens a text file.
- Final API 36 x86_64 preview APK emulator close proof, driven from TerminalScreen Stop (not SpacesTree raw close): E2E-Pane split closed one pane without confirmation and left the sibling; E2E-Tab confirmed Close tab? and left the Keep tab; E2E-Workspace confirmed Close tab? then Close workspace? and the workspace disappeared; E2E worktree group confirmed tab, workspace, then Close worktree group? for two workspaces, Cancel preserved both plus the live fixture, and retry/confirm removed both. Temporary EACCES/ECONNRESET/ETIMEDOUT revalidation stays retryable at plugin and host boundaries. The close-flow source fingerprint was unchanged during that proof. Thirteen `close-*.png` label-only screenshots are in the watched attachment dirs.
...
- 2026-08-29 — Mark tested after TerminalScreen Stop emulator proof across pane, tab, workspace, and worktree-group fixtures, including Cancel-then-confirm on the group and retryable EACCES/ECONNRESET/ETIMEDOUT revalidation.
- 2026-08-29 — Pin kernel Agent close to the packaged Workspace Hierarchy `agent.close` capability and guarded write pipeline. Live refusal races now advance beyond both attempted and confirmed scopes; failed revalidation returns Retry/error instead of falsely reporting an already-closed target.
- 2026-08-29 — Collapse mobile, plugin, and realtime Agent presentation onto one `AgentInfo` contract. The Herdr adapter maps `name`, `title`, `agent`, `display_agent`, `agent_status`, and route-generation readiness one-to-one; all consumers read the same properties and no longer derive identity or work from pane labels, terminal titles, lifecycle caches, or provider-based title fallbacks.
- 2026-08-29 — Move Agent naming/title policy and exact close escalation behind two backend plugins. Pane Titler writes canonical Herdr `name` and session-current `title` while preserving provider fields. Workspace Hierarchy adds a declared close write that delegates live scope decisions to Herdr, returns typed confirmation/retry outcomes, and never leaves pane, tab, workspace, worktree-group, already-gone, or temporary-outage cases as raw mobile dead ends.
...
- 2026-08-15 — Reopened to make primitives and slots first-party platform concepts, remove feature-specific shell ownership, harden runtime lifecycle/performance, and prove the public contract on Android.
...
- 2026-08-16 — Realtime provider policy moved entirely behind public `host.stream`: React Native keeps generic PCM/WebRTC capabilities and provider-blind frames; the bundled backend adapts xAI Grok, runs tools next to Herdr, and fences revocation/admission/backpressure. Large attachments now stream bounded encrypted chunks to device storage. Removed Usage, Machine, and Example clutter from the home navigation strip.
