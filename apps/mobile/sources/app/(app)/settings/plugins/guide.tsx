...
export default function PluginGuide() {
    return <ItemList>
        <ItemGroup title="Where plugins live" footer="Plugins run on your computer through Herdr. Their muxr screens and controls appear after connection.">
            <Item title="Built-in categories" subtitle="Agent workflow, files, terminal, voice, and machine status" subtitleLines={0} showChevron={false} />
        ...
        <ItemGroup title="Task titles" footer="Task titles name the work. Agent identity names who is doing it. Manual renames always win.">
            <Item title="Another title writer active?" subtitle="Use Switch in Task titles, or keep the existing writer. Restore it later from the same detail." subtitleLines={0} showChevron={false} />
            <Item title="Offline or unsure?" subtitle="Existing titles stay. Preview is read-only and can explain rejected samples." subtitleLines={0} showChevron={false} />
        </ItemGroup>
