...
| 4 Funnel, parity and canonical docs | done | Unit 4 at 8fb01831; reopened by the 2026-09-12 QR amendment and closed at product source commit 9b6f59cf (web QR producer, cold intake, web scanner with camera lifecycle/remount ownership, demo handoff, canonical copy; Astra finding 1 repaired in 9bb4f038, 6951ba04, 9b6f59cf) — see pwa-web-qr-implementation.md |
| 5 Functional freeze and candidate gate | done (QR refreeze) | prior tuple 319a80c0 (tarball `sha512-0tJ5zq…`, export tree `bd1c5f17…`) is superseded and must not be used. Frozen QR tuple: product source commit `9b6f59cf67bd87a7abe11d3dae4fe9402ff2fd1a`, tree `388b1336dcbad439db20429fb20e6b062ba33b80`; candidate tarball 14254106 bytes, sha256 `b794f3ed52e210147f1a61ab722723a4190436610212171abf695da81e77b954`, npm integrity `sha512-myWAuNt+aTnIBpvR8nDnuT8R36B8BPnc7g0PzbTX7P85++Y7imeRiw7qBmuz8P8tkfCltXMfCUJEYbWdgprU8g==`, npm shasum `a3dbc5ddf6a40303234698e8727aade675a0333f`; web export index sha256 `9edd379ead99e313d92c2927ffa2889b4d46b433cf19c0534eeaf74b029ad6e6`, web export tree sha256 `d215d536587429c467d66e12e69bfe60c1a69b25928d9833ff94f2125244aeb1`. Evidence against this tuple: browser QR acquisition flow (`checkDemoFlow.mjs`) passed including camera lifecycle, Back/Forward and Connect remount; clean-room Herdr plugin install/Setup/pair/update/rollback/uninstall gate 21/21 passed against this tarball. The owner explicitly overrode the prior complete-suite requirement for this refreeze: integration/E2E evidence only; the broad suite was stopped and is not evidence. This row was recorded in a later docs-only commit, not in the product source commit. |
| 5a Stale-cert findings re-verified on 8eac274c (Astra direction-pwa D2/D3) | closed | Both red findings from the 9b6f59cf ecosystem lanes were re-tested against the 8eac274c candidate (tree `c21e0624356208a4920d54bbf11334952520a81f`; tarball sha256 `7402ddf7e783ff2b7289c940d0e34fd16e026ed504a3b9ff74fdafcb7ecd57ef`, npm integrity `sha512-45cI5/+BLAsqlHXVEiNLIsYKKu3atyea/joZkJPr19/BjGAvHoY47GX9ipYBYGHNWYs3hwGf/Q1zJ60CbxSocw==`; web index sha256 `e30e7cd112513bab0de1c3b09385513d56d5556b0cd0a847929c58f40228c730`). **A (consent named "this machine", no lifetime):** fixed — source-verified (`pairingLocator` appends `name=`, parser accepts it, `pairingConsent.spec.ts` drives the CLI locator through the app parser) and shipped-package-verified (`setup/domain/dist/pairing.js`), plus a live browser consent reading the real computer name with "Control · expires in eight hours"; native consent states "Full control · until revoked", which is the truthful lifetime because native grants are indefinite on host, relay and app. Native on-device render was not re-run. **B (one failed send + one retry executed twice):** closed on the live result — the byte-identical relay-pause → send → timeout → resume → unchanged restored draft → one Send sequence produced executions = 1 (deliveries file, real Herdr pane scrollback, host receipt ledger with one `done` receipt); the source trace and six independent readers are supporting evidence only. Evidence: `~/.muxr/attachments/pane/w1FE:p8N/tip-cert/` (verdict.md, identity.json). |
| 5b Terminal status truthfulness (direction-pwa D4) | done | An open terminal socket no longer reads as "live": the channel folds the machine transport's known failures in (`OpenTerminal.ts`), so a timed-out request or lost route shows **Connection unconfirmed** (pill + accessibility line) until this pane paints again or the host answers; the healthy state reads "connected"; Reconnecting is reserved for an actual re-attach. No heartbeat added. Flow check in `OpenTerminal.spec.ts`; live pause journey re-run recorded in the same evidence directory. |
| 5c Browser/Code lane boundary (direction-pwa 23:24, D1) | recorded | Surfaces lane works only in `feat/pwa-surfaces` (foundation `bcb2e1d6`, `27ac0740`, `c8156bb7` on top of `8eac274c`; preserved on remote `backup/pwa-surfaces-c8156bb7`) and hands coherent commit ranges with base hashes; this lane is the sole integrator/pusher of `feat/pwa-primary-channel` and the website lock. Ownership here: `apps/relay/src/**`, `scripts/setup/{domain/pairing.ts,infrastructure/**}`, release/diagnostic packaging, pairing storage/application and `pair.tsx`, package manifests/lockfile, website worktree, shared session-screen wiring (current terminal drafts/status retained). Frozen interfaces this lane implements against: `preview.bootstrap(lease)` (origin/endpoint generation/one-use POST material), `preview.renew/release` (authority checks retained); `browser-session` offers under the distinct `surface.browser.control-host-session` authority (ordinary Preview stays `surface.browser.open`); `browser.session.{status,take,pause,resume,return,signal}` bound to an enrolled browser-service grant supplied by the pairing adapter (never a shared broadcast key). Foundation already touches two owned files (`pairing/infrastructure/muxrClient.ts` +21 offer listener, `apps/relay/src/routing/infrastructure/preview.ts` +61) — carried at integration. |
| 5d Browser/Code integration and packaged candidate (direction-pwa 23:24, D9) | candidate packaged; device QA in progress | 13 surfaces parcels integrated at `59263812` (tree == `feat/pwa-surfaces@1706dcce` + this lane's fixes); packaging fix `681ff38a` (shipped plugins resolve the bundled crypto.js/contract.mjs). Tarball `681ff38a` sha256 `ed8eae63a94c4061f26675652c734425e49cdcbea51c69e51dd41d67d9b259b7`; native Android APK from `59263812` sha256 `63317f17ab4f4c5b9b79829cbe764f05e730f1898f72586de253508afdf233b2` (vc54, arm64, release keystore). Directive 7 re-run on the packaged `59263812`: executions = 1. Candidate stack served to the tailnet from a copy of the owner's state for device QA. Known limit: in-app dev Preview needs per-endpoint origins with phone-trusted TLS and a relay CSP change — pending the tailnet-only re-cut. Site lock still names `8eac274c` and is not the Browser candidate. |
