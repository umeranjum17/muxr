...
| `MUXR_BIND_HOST` | `auto` · `127.0.0.1` · `0.0.0.0` | `auto` | single-machine, shared-relay | relay and host | Which address the relay listens on. auto is loopback for Tailscale Serve / external HTTPS / legacy tunnel routes and all interfaces for direct/private/LAN routes; an explicit 127.0.0.1 or 0.0.0.0 is never silently widened. The resolved address shows in inspection. |
