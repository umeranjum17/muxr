...
function KeyRow({ contribution, channel }: { contribution: PluginTerminalKeyRow; channel?: TerminalChannel }) {
    ...
    const style = (selected = false) => ({ minWidth: 44, alignItems: 'center' as const, paddingHorizontal: 10, paddingVertical: 5, borderRadius: 6, backgroundColor: selected ? theme.colors.accent : theme.colors.surfaceHigh });
    return <><Pressable onPress={() => setCtrl(!ctrl)} accessibilityRole="button" accessibilityLabel="Control" accessibilityState={{ selected: ctrl }} style={style(ctrl)}><Text style={{ color: ctrl ? '#fff' : theme.colors.text }}>ctrl</Text></Pressable><Pressable onPress={() => setShift(!shift)} accessibilityRole="button" accessibilityLabel="Shift" accessibilityState={{ selected: shift }} style={style(shift)}><Text style={{ color: shift ? '#fff' : theme.colors.text }}>shift</Text></Pressable>{contribution.keys.map((key) => <Pressable key={resolvePluginText(key.label)} accessibilityRole="button" accessibilityLabel={resolvePluginText(key.accessibilityLabel)} onPress={() => { channel?.sendText(ctrl && shift ? key.ctrlShift ?? key.ctrl ?? key.shift ?? key.send : ctrl ? key.ctrl ?? key.send : shift ? key.shift ?? key.send : key.send); setCtrl(false); setShift(false); }} style={style()}><Text style={{ color: theme.colors.text }}>{resolvePluginText(key.label)}</Text></Pressable>)}</>;
}
