...
The Usage screen — host product code, not a plugin — runs the exact pinned [ccusage](https://github.com/ccusage/ccusage) backend offline against local coding-agent logs, leads each provider tab with its plan's quota windows, and shows today's allowlisted per-agent token totals — with estimated costs and bounded model labels — without prompts, project details, or session details. It shows tabs only for integrated providers — measured activity this week or a connected plan/account — says when a plan is not connected rather than claiming it does not exist, and shows an explicit empty state when no supported provider is detected. The Home Right now card is a compact view of the same host collection that supplies every Usage tab; opening a tab reuses its measured limits and activity rather than separately polling each plan. While Usage or Home is focused in the foreground, the app re-collects in the background every 15 minutes; the Usage header, Home card, and pull-to-refresh can request a fresh read immediately, with a short guard against repeated forced reads. If a Usage report read fails, its last attempt and retry remain visible across screen visits until a report answers; limits from the card alone are not treated as a measured-activity report. npm installs one platform-specific ccusage binary (about 4 MB) for Linux or macOS. Plan windows come from each provider's local integration (Claude snapshot/endpoint, Codex app-server, OpenCode Go, Z.ai via Pi); other coding CLIs are never invoked.
