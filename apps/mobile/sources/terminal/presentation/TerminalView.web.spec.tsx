...
const link = vi.hoisted(() => ({
    options: null as null | { linkHandler: { activate: (event: MouseEvent, url: string) => void; hover: (event: MouseEvent, url: string) => void; leave: () => void } },
...
it('opens the action card for OSC 8 labels held on the browser terminal while taps keep their card', () => {
    ...
    const screen = {
        ...
        dispatchEvent(event: { type: string }) {
            ...
            if (event.type === 'mousemove') link.options?.linkHandler.hover(event as MouseEvent, 'https://example.test/osc8');
        },
    };
    const host = {
        style: {},
    ...
    vi.stubGlobal('MouseEvent', class { constructor(public type: string, public coordinates?: unknown) {} });
    ...
    expect(reached).toHaveBeenCalledWith('https://example.test/osc8', { x: 24, y: 18 });
