...
import { createDomainStores } from './index.js';
...
async function runSelfCheck(): Promise<void> {
    ...
    try {
        ...
        const afterGhostRestart = createDomainStores({ dataDir, now }).attention.catalog();
        assert(
        ...
        const afterCorruptUnread = createDomainStores({ dataDir, now }).unread.catalog();
        ...
        const afterCorruptAttention = createDomainStores({ dataDir, now }).attention.catalog();
