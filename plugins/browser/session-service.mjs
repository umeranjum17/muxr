...
// The sealed WebRTC hop can be a few hundred milliseconds behind the local
// permit pulse, especially for a PWA on a throttled mobile connection.
const PERMIT_TTL_MS = 1_000;
...
async function launchChrome(dir, port, token, extension) {
    ...
    const args = [
        ...
        // Headless Chrome otherwise reports an 800x600 virtual display and
        // silently clamps the tall phone window, padding its tab capture.
        ...(process.env.MUXR_BROWSER_HEADLESS === '1' ? ['--headless=new', '--ozone-override-screen-size=1920,1080'] : []),
...
class BrowserService {
    ...
    async open({ context, name }) {
        ...
        const session = {
            ...
            pendingCandidates: [],
            // The first viewer is usually a phone. Start the agent's real
            // tab at a readable phone width; a device that takes the seat
            // still supplies its own viewport through hello below.
            viewport: { width: 390, height: 700, scale: 1 },
    ...
    navigate(session, url) {
        ...
        return this.agentOp(session, async () => {
            try {
                ...
                await this.cdp.waitFor('Page.frameStoppedLoading', (params, sessionId) => sessionId === session.cdpSession && params.frameId === result.frameId, 15_000);
                ...
                session.reason = /ERR_CONNECTION_REFUSED/i.test(detail) && /^https?:\/\/(localhost|127\.0\.0\.1|\[::1\])(?::|\/|$)/i.test(target)
                    ? 'The dev server is not running on this computer.'
                    : /ERR_NAME_NOT_RESOLVED/i.test(detail)
                        ? 'The page address could not be found.'
                        : /timed out|timeout/i.test(detail)
                            ? 'The page did not finish loading.'
                            : 'The page could not be loaded.';
                this.record(session, { by: 'service', note: session.reason });
    ...
    click(session, target) {
        return this.agentOp(session, async () => {
            ...
            await sleep(150);
            if (session.captured) await this.reportFocus(session);
            return {};
        });
    }
    ...
    async help(session) {
        if (session.state !== 'agent-driving') throw this.statusError(session);
        // The agent may ask the user to complete a field in the live tab.
        // Asking does not fence either actor or replace the captured target.
        this.record(session, { by: 'agent', note: 'asked the user to act in the shared browser' });
        this.touch(session);
    ...
    /** The agent and a paired control device share this captured tab. */
    watching(session) {
    ...
    async signal(session, deviceId, generation, message) {
        ...
        const entry = this.identity.devices[deviceId];
        if (entry === undefined || entry.expiresAt <= now()) throw new Error('this device is not paired with the browser service');
        const current = session.device;
        const device = current?.deviceId === deviceId && current.keyVersion === entry.keyVersion
            ? current
            : { deviceId, keys: deriveBrowserSessionKeys(entry), keyVersion: entry.keyVersion, sender: newV2SenderState(), replay: newV2ReplayTracker() };
        ...
        // A new authorized peer may replace the old one only after its sealed
        // message opens. Stop capture and permits before accepting its input.
        if (current !== device) {
            if (current !== undefined) await this.stopPeer(session);
    ...
    async handleSignal(session, device, message) {
        ...
        switch (type) {
            case 'hello': {
                ...
                // Watching never changes the host viewport; it belongs to the
                // agent's tab and both actors use its current geometry.
            ...
            case 'presented': {
                ...
                } else if (this.watching(session) && session.captured) {
                    // The device has displayed this generation's real frame.
                    // Permit input on this peer without taking the agent's seat.
                    this.startPermits(session);
                }
    ...
    startPermits(session) {
        ...
        session.permitTimer = setInterval(() => {
            const deviceGrant = session.device === undefined ? undefined : this.identity.devices[session.device.deviceId];
            if ((session.state !== 'you-control' && !this.watching(session)) || session.generation !== generation
                || deviceGrant === undefined || deviceGrant.expiresAt <= now()) { clearInterval(session.permitTimer); return; }
    ...
    onExtensionEvent(event) {
        ...
        if (event.event === 'input') {
            ...
            else if (this.watching(session)) void this.stopPeer(session).catch(() => undefined);
    ...
    /** Device input shares the agent operation queue, with a live permit and fresh geometry. */
    /**
    ...
    async onInput(session, intent) {
        ...
        // The permit is a liveness proof at receipt. The agent may already
        // have a queued navigation or click; do not let its duration expire a
        // valid tap before dispatch. State and target are checked again then.
        ...
        const task = session.queue.then(() => this.dispatchInput(session, intent, generation, deviceId));
        session.queue = task.catch(() => undefined);
        await task;
    }

    async dispatchInput(session, intent, generation, deviceId) {
        if (session.generation !== generation || (session.state !== 'you-control' && !this.watching(session)) || !session.captured) return;
        if (deviceId === undefined || session.device?.deviceId !== deviceId) return;
        const activeDevice = this.identity.devices[deviceId];
        if (activeDevice === undefined || activeDevice.expiresAt <= now()) return;
    ...
    revoke({ deviceId }) {
        ...
        for (const session of this.sessions.values()) {
            ...
            if (session.device?.deviceId === deviceId) {
                session.device = undefined;
                if (this.watching(session)) void this.stopPeer(session).catch(() => undefined);
            }
        }
        return {};
    }
