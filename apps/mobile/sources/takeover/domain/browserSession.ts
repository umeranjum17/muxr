...
/** Permits renew every 100 ms and die after one second; input without a live one is dropped locally. */
export const PERMIT_TTL_MS = 1_000;
...
export function ownershipStrip(state: BrowserSessionState | 'connecting' | 'needs-pairing'): OwnershipStrip {
    switch (state) {
        case 'agent-driving': return { label: 'Live · Agent and you', icon: 'sparkles-outline', tone: 'agent' };
        case 'waiting-for-you': return { label: 'Live · Your help needed', icon: 'hand-left-outline', tone: 'attention' };
        ...
        case 'checking': return { label: 'Reconnecting browser', icon: 'refresh-outline', tone: 'attention' };
