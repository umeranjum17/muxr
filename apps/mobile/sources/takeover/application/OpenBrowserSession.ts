...
export function openBrowserSession(command: OpenTakeoverCommand): TakeoverSession {
    ...
    const freshMedia = (): boolean => peer !== undefined && presentedGeneration === mediaGeneration && presentedFor === generation();
    const geometryCurrent = (): boolean => geometry !== undefined && geometry.generation === generation();
    const sharedInput = (): boolean => status?.state === 'agent-driving' || status?.state === 'waiting-for-you';
    const inputUnlocked = (): boolean => phase === 'live' && transition === undefined && (sharedInput() || ownsSeat(status))
    ...
    const connectMedia = async (): Promise<void> => {
        ...
        presentationTimeout = setTimeout(() => {
            if (!current() || freshMedia()) return;
            mediaGeneration += 1;
            failure = 'The agent browser stream did not show a page.';
            closePeer();
            notify();
        }, 30_000);
        try {
    ...
    const refresh = async (): Promise<void> => {
        ...
        try {
            ...
            if (phase === 'checking' && rechecks < 15) {
                ...
                failure = 'Connection to the agent browser was lost.';
                notify();
            }
        }
    };
    ...
    const release = (): void => {
        if (ownsSeat(status) || sharedInput()) send({ type: 'release', generation: generation() });
    };
    ...
    const heartbeat = setInterval(() => {
        if (closed || (!ownsSeat(status) && !sharedInput()) || peer === undefined) return;
    ...
    return {
        ...
        presented: (forMedia) => {
            ...
            if (firstForSeat && (status?.state === 'taking-control' || sharedInput())) {
