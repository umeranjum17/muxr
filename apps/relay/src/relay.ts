        ...
        void (async () => {
            ...
            if (config.localAuthority && localPairing !== undefined && url.pathname.startsWith('/v1/selfhost/pair-sessions')) {
                ...
                if (req.method === 'POST' && url.pathname === '/v1/selfhost/pair-sessions') {
                    ...
                    const durable = deviceKind === 'browser' && deviceAuthority === 'control' && body?.durable === true;
                    ...
                    const session = await localPairing.createSession({ claim, machineSlug, deviceKind, authority: deviceAuthority, ...(personal ? { personal: true } : {}), ...(durable ? { durable: true } : {}) });
    ...
    if (config.advertiseMdns) {
        try {
            ...
            const service = bonjour.publish({
                ...
                host: `muxr-${hostname().toLowerCase().replace(/[^a-z0-9-]/g, '-').slice(0, 40)}-${listeningPort}`,
                type: 'muxr',
                protocol: 'tcp',
                port: listeningPort,
