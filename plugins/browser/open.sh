#!/usr/bin/env bash
# Tools entry for the Browser plugin: open a blank browser beside the focused
...
MUXR_CLI="$(resolve_muxr)" || { echo "browser: the muxr CLI was not found; install @trymuxr/cli or set MUXR_BIN" >&2; exit 1; }
exec "$MUXR_CLI" browser open --beside -- about:blank
