...
**No — not on today's contract, and the blocker is one specific thing.**

### The blocker

**Herdr plugin actions cannot return data.** `runtime.rs:104-180` starts the process and returns a `Running` log immediately. There is no stdin, no typed input, no result, no schema — only a capped stdout you poll for via `plugin.log.list`. A UI needs `list the files` → `[…]` in one bounded round trip. Herdr's action contract structurally cannot do that.
...
|---|---|---|---|
| 1 | **Request/response actions in Herdr** — stdin JSON in, bounded JSON stdout out, awaited result on `plugin.action.invoke` | Herdr (`runtime.rs`, `plugins.rs`) — an additive `mode`/`await` variant next to the existing fire-and-forget path | Large. Herdr change, not muxr's to make. Blocks everything below. |
...
| 4 | ~~A Herdr pane slot on the phone~~ **Closed by `muxr.panes`** | bundled plugin + existing session route | Enabled `[[panes]]` contributions open through `herdr plugin pane open`; every resulting agent-less pane already routes as `shell:<paneId>`. |
