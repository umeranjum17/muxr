...
const adjectives = [
    'clever', 'sunny', 'swift', 'bright', 'calm',
...
/*
 * The host does the git work: per-step RPCs mean a retry after a mid-failure
 * resumes instead of restarting, a dirty base checkout is reported as a file
 * list instead of raw stderr, and nothing here ever runs with its cwd inside
...
export function landWorktree(
    ...
    return sync.request('worktree.land', { worktreePath, message, stash });
}
