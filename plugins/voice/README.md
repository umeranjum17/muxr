...
Provider policy is host-owned: `muxr voice` on the computer (or the Herdr pane **muxr host voice**) chooses the adapter and, for API-key providers, stores the key owner-only. A computer with no saved choice defaults to Codex Voice (experimental); explicit saved choices and migrated legacy choices remain selected. Codex uses the computer's ChatGPT CLI login (`codex login`), not an API key; login readiness does not guarantee realtime subscription entitlement. Clients see only Ready / Not configured on this computer.
