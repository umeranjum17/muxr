 ...
 * - `muxr browser session open [http(s)-url]` (or the other session verbs)
...
export async function runSurfaceCli(argv) {
    ...
    try {
        ...
        if (command === 'browser') {
            ...
            usageError('usage: muxr browser open|home|update|reload|close|origin|session ...');
        }
        if (command === 'code') {
            const [sub, ...rest] = args;
...
async function runBrowserSession(args, json) {
    ...
    if (verb === 'open') {
        const parsed = splitArgs(rest, { flags: ['--beside', '--focus'], options: ['--name', '--provider'] });
        if (parsed.positionals.length > 1) usageError('usage: muxr browser session open [http(s)-url] [--beside|--focus] [--name NAME] [--provider ID]');
        const opened = await callSurfaceBroker({
            method: 'browser.session.open',
            ...nameOf(parsed),
            ...placementOf(parsed),
            ...(parsed.options['--provider'] === undefined ? {} : { provider: parsed.options['--provider'] }),
        });
        const result = parsed.positionals[0] === undefined ? opened : await callSurfaceBroker({
            method: 'browser.session.navigate', url: parsed.positionals[0], ...nameOf(parsed),
        });
