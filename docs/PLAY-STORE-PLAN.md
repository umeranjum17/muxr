...
Status: **implementation-ready, submission blocked on signing, public release, clean-room evidence, and Play Console work.** Production package ID is permanently `com.trymuxr.app`.
...
- Version name: `0.1.0`; version code: `1` only if Play Console has never seen this package. Every subsequent upload must increment it.
...
- **App access:** no paid credentials. Explain that muxr is a companion to a user-run computer and provide exact English setup commands from the public release. Include a reviewer-safe disposable repository and no provider key. Pairing claims are intentionally one-use and short-lived; never place a permanent privileged QR/token in Play Console. If review cannot follow the companion setup, add a bounded non-privileged review fixture only after an actual rejection—not pre-emptively.
