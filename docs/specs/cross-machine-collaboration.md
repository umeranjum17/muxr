...
- 2026-08-26: cross-relay evidence proved the Linux outbound peer opened against the wrong relay: the source relay delivered locally while the target Mac relay and host saw nothing. Pairing now carries the target machine's verified relay URL into authorization; the target signs it into the install bundle; the source persists it and uses that same pinned endpoint for grant refresh, ticket issue, and WebSocket creation. Local Tailscale hosts derive their owned public relay endpoint from their current ingress instead of stale copied relay state. Two-distinct-relay coverage proves the source relay receives zero connections while the target receives and completes liveness.
