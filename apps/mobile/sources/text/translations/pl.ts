...
export const pl: TranslationStructure = {
    ...
    plugins: {
        ...
        limits: {
            ...
            paceLimited: 'Ograniczenie',
            paceExhausted: 'Wyczerpano',
            paceOnTrack: 'Zgodnie z tempem',
            paceAhead: 'Poniżej tempa',
            paceBurning: 'Szybkie zużycie',
            percentLeft: ({ percent }: { percent: number }) => `pozostało ${percent}%`,
            percentUsed: ({ percent }: { percent: number }) => `wykorzystano ${percent}%`,
            runsOutIn: ({ time }: { time: string }) => `skończy się za ${time}`,
            asOf: ({ time }: { time: string }) => `Limity na ${time}`,
        },
        rightNow: {
            title: 'Teraz',
            ...
            refreshIn: ({ seconds }: { seconds: number }) => `za ${seconds} s`,
    ...
    spacesTree: {
        ...
        shell: 'terminal',
        childEmpty: 'pusty',
        childAgents: ({ count }: { count: number }) => `${count} ${plural({ count, one: 'agent', few: 'agenty', many: 'agentów' })}`,
        noMatches: 'Brak pasujących przestrzeni',
        expand: 'Pokaż więcej',
        collapse: 'Pokaż mniej',
        openLabel: ({ label, line2 }: { label: string; line2: string }) => `Otwórz ${label}, ${line2}`,
    },
    ...
    commandPalette: {
        ...
        yourCommands: 'Twoje polecenia',
        editQuickActions: 'Edytuj odpowiedzi i polecenia…',
    },
    ...
    files: {
        ...
        nextFileNamed: ({ title, ordinal, total }: { title: string; ordinal: number; total: number }) => `Następny plik, ${title}, ${ordinal} z ${total}`,
        filePosition: ({ current, total }: { current: number; total: number }) => `Plik ${current} z ${total}`,
