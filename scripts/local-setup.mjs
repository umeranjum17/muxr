...
function hostedAuthReport(complete, expires, pendingValid) {
    const credentialIsLive = complete && Number.isFinite(expires) && expires > Date.now();
...
export async function runDoctor() {
    ...
    if (existsSync(authPath()) || managedMode === 'hosted') {
        try {
            ...
            const report = hostedAuthReport(complete, expires, pendingValid);
