...
| The link says the grant expired or access was removed | (in the browser) status pill | **Pair again** | `muxr pair --browser` on the computer and open the new link in that browser |
| The pairing link does not open, or the browser blocked a pop-up | copy the printed link | the pairing page with the computer name | paste it into the address bar of the browser you want to pair; links are one-use and expire after a few minutes, so run `muxr pair --browser` again if needed |
| The installed iPhone app asks to pair although Safari was paired | — | — | expected: the installed app has its own storage; run `muxr pair --browser` and open the link inside the app |
