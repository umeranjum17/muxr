...
import { surfaceLabel, useShownSurface } from '@/preview';
...
const PaneCard = React.memo(function PaneCard(props: {
    ...
    // A Browser shown in this pane's session is that pane's content: the
    // safe label only, never a page image, and nothing at all of a private
    // handover beyond the fact of it.
    const machineId = getCachedConnectionSettings().machineId;
    const sessionId = pane.sessionId ?? '';
    const shown = useShownSurface(sessionId);
    const surfaces = useSurfaceEntries(machineId, sessionId);
    const shownSurface = shown === null ? undefined : surfaces.find((entry) => entry.offer.name === shown);
        ...
        >
            <View style={{ flexDirection: 'row', alignItems: 'flex-start' }}>
                ...
                <Pressable
                    ...
                    disabled={!props.canClose || props.pending}
                    hitSlop={4}
                    accessibilityRole="button"
            ...
            {shownSurface !== undefined ? (
                <View style={{ height: PREVIEW_HEIGHT, marginHorizontal: 10, marginBottom: 10, borderRadius: 8, backgroundColor: theme.colors.surfaceHigh, alignItems: 'center', justifyContent: 'center', gap: 6, padding: 10 }}>
                    <Ionicons name="globe-outline" size={20} color={theme.colors.textSecondary} />
                    <Text numberOfLines={2} style={{ ...Typography.default(), fontSize: 11, color: theme.colors.textSecondary, textAlign: 'center' }}>
                        {shownSurface.offer.kind === 'browser-session' ? 'Agent browser session' : surfaceLabel(shownSurface.offer)}
                    </Text>
                </View>
            ) : (
