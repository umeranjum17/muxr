...
const chips: Array<[keyof typeof glyphs, string]> = [['desktop', 'Machine'], ['file', 'Files'], ['inbox', 'Inbox'], ['branch', 'Ports'], ['play', 'Runbooks']];
const NavChips = () => (
...
const Item: React.FC<{ title: string; subtitle: string; detail?: string; dot?: string; first?: boolean }> = ({ title, subtitle, detail, dot, first }) => <div style={{ minHeight: 74, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 10, background: c.surface, borderTop: first ? undefined : `1px solid ${c.divider}` }}>{dot ? <Dot color={dot} size={9} /> : null}<div style={{ flex: 1 }}><div style={{ font: `15px ${SANS}` }}>{title}</div><div style={{ marginTop: 4, color: c.textSecondary, font: `13px/18px ${SANS}` }}>{subtitle}</div></div>{detail ? <span style={{ color: c.textSecondary, font: `13px ${SANS}` }}>{detail}</span> : null}</div>;
export const ConnectionScreen = () => <div style={{ position: 'absolute', inset: 0, background: c.grouped, color: c.text }}><div style={{ height: 70, display: 'flex', alignItems: 'center', gap: 14, padding: '8px 16px' }}><Round><Icon name="back" size={25} /></Round><strong style={{ font: `600 20px ${SANS}` }}>Connection</strong></div><div style={{ padding: '14px 20px 7px', color: c.textSecondary, font: `600 13px ${SANS}` }}>STATUS</div><div style={{ margin: '0 14px 14px', borderRadius: 14, overflow: 'hidden' }}><Item first dot={c.success} title="Connected" subtitle="Your machine is reachable from this device" /><Item title="Transport" subtitle="HTTPS/WSS transport + end-to-end encryption" detail="Self-host" /><Item title="Relay" subtitle="wss://your-host.ts.net" /><Item title="Versions" subtitle="app 0.1.13 · host 0.1.13" /></div><div style={{ padding: '8px 20px 7px', color: c.textSecondary, font: `600 13px ${SANS}` }}>YOU RUN THE RELAY</div><div style={{ margin: '0 14px', borderRadius: 14, overflow: 'hidden' }}><Item first title="End-to-end encrypted" subtitle="Terminal output, keystrokes and approvals are sealed to your machine key before they leave this phone" /><Item title="Pairing holds the credentials" subtitle="Keys and tokens come from the QR or short-code grant. There is no shared-key fallback." /></div></div>;
